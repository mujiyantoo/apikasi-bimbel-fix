...
export async function GET(request) {
  try {
    ...
    const formatted = pendaftaran.map(p => ({ ...p, id: p._id.toString() }))
    return NextResponse.json(formatted, { headers: corsHeaders })
  } catch (error) {
    return NextResponse.json({ error: 'Gagal memuat data' }, { status: 500, headers: corsHeaders })
  }
}
  ...
  try {
    ...
    if (countFromIP >= 2) {
      return NextResponse.json(
        { error: 'Batas pendaftaran tercapai. Maksimal 2 kali dari perangkat yang sama.' },
...
export async function DELETE(request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'ID tidak ditemukan' }, { status: 400, headers: corsHeaders })
    ...
    if (result.deletedCount === 0) return NextResponse.json({ error: 'Data tidak ditemukan' }, { status: 404, headers: corsHeaders })

    return NextResponse.json({ message: 'Data berhasil dihapus' }, { headers: corsHeaders })
  } catch (error) {
    return NextResponse.json({ error: 'Gagal menghapus' }, { status: 500, headers: corsHeaders })
  }
}
