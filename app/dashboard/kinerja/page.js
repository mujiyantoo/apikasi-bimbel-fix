...
export default function KinerjaPage() {
  ...
  const fetchPegawai = async () => {
    try {
      ...
      setPegawai(list.filter(p =>
        p.jabatan?.toLowerCase().includes('guru') ||
        p.jabatan?.toLowerCase().includes('tutor') ||
        p.jabatan?.toLowerCase().includes('pengajar')
      ))
    } catch (error) {
      console.error('Error:', error)
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Kinerja berhasil ditambahkan! Gaji: Rp ${result.gaji.toLocaleString('id-ID')}`)
        setIsDialogOpen(false)
        resetForm()
        fetchKinerja()
      } else {
        toast.error(result.error || 'Gagal menambahkan kinerja')
      }
    } catch (error) {
      toast.error('Gagal menambahkan kinerja')
    }
  }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success('Data kinerja berhasil dihapus')
      fetchKinerja()
    } catch (error) {
      toast.error('Gagal menghapus kinerja')
    }
  }
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select value={formData.kategori} onValueChange={(v) => setFormData({ ...formData, kategori: v })} required>
                      ...
                      <SelectContent>
                        <SelectItem value="Reguler">Reguler (flat per sesi)</SelectItem>
                        <SelectItem value="PR">PR (dihitung per menit)</SelectItem>
                      </SelectContent>
                    </Select>
                    {formData.kategori === 'PR' && (
                      <p className="text-xs text-orange-500 mt-1">PR dihitung berdasarkan durasi mengajar</p>
                    )}
                  </div>
                </div>
                <div>
